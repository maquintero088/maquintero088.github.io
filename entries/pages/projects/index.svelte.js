import { c as create_ssr_component, d as each, b as add_attribute, e as escape } from "../../../chunks/index-38cedc01.js";
...
const Projects = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<div class="${"todos svelte-16nsat"}"><h1>Todos</h1>

	<form class="${"new svelte-16nsat"}" action="${"/projects"}" method="${"post"}"><input name="${"text"}" aria-label="${"Add todo"}" placeholder="${"+ tap to add a todo"}" class="${"svelte-16nsat"}"></form>

	${each(todos, (todo) => {
    return `<div class="${["todo svelte-16nsat", todo.done ? "done" : ""].join(" ").trim()}"><form action="${"/projects?_method=PATCH"}" method="${"post"}"><input type="${"hidden"}" name="${"uid"}"${add_attribute("value", todo.uid, 0)} class="${"svelte-16nsat"}">
			...
			<form class="${"text svelte-16nsat"}" action="${"/projects?_method=PATCH"}" method="${"post"}"><input type="${"hidden"}" name="${"uid"}"${add_attribute("value", todo.uid, 0)} class="${"svelte-16nsat"}">
			...
			<form action="${"/projects?_method=DELETE"}" method="${"post"}"><input type="${"hidden"}" name="${"uid"}"${add_attribute("value", todo.uid, 0)} class="${"svelte-16nsat"}">
